    ...
    def get_user_tweets(self, username, max_results=10):
        ...
        try:
            ...
            if not tweets or not tweets.data:
                logger.info(f"用户 {username} 没有可访问的推文，使用模拟数据")
                return self.get_mock_tweets(username)
...
def fetch_researcher_content(researcher_id):
    ...
    try:
        ...
        logger.info(f"开始获取 {name} ({x_account}) 的内容")
        ...
        return jsonify({
            'message': f'成功获取 {name} 的内容，新增 {new_content_count} 条',
            'new_content_count': new_content_count,
            'total_fetched': len(tweets)
        })

    except Exception as e:
        logger.error(f"获取研究者内容失败: {e}")
