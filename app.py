	...
	class TwitterAPI:
	    ...
	    def get_user_tweets(self, username, max_results=10, start_time=None, end_time=None):
	        ...
	        try:
	            ...
	            # 限制最大结果数
	            max_results = min(max_results, 50)  # 减少最大结果数，避免请求过多
	            # 获取推文
	            logger.info(f"🔄 开始请求推文数据...")
	            ...
	            if not tweets_response or not tweets_response.data:
	                ...
	                return []
	            logger.info(f"✅ 成功获取 {username} 的 {len(tweets_response.data)} 条推文")
	            # 处理媒体信息
	            media_dict = {}
	...
	# 添加一个初始化方法，在应用启动时自动更新所有研究者的用户信息
	def initialize_researcher_data():
	    """初始化研究者数据，包括更新用户信息"""
	    if not researcher_manager or not twitter_api or not twitter_api.client:
	        logger.warning("⚠️ 系统组件未完全初始化，跳过研究者数据初始化")
	        return
	    try:
	        conn = sqlite3.connect('research_platform.db')
	        cursor = conn.cursor()
	        # 获取所有研究者
	        cursor.execute('SELECT id, name, x_account FROM researchers')
	        researchers = cursor.fetchall()
	        logger.info(f"🔄 开始初始化 {len(researchers)} 位研究者的用户信息...")
	        updated_count = 0
	        failed_count = 0
	        ...
	        conn.commit()
	        conn.close()
	        logger.info(f"✅ 研究者数据初始化完成: 成功 {updated_count} 个，失败 {failed_count} 个")
	    except Exception as e:
	        logger.error(f"❌ 初始化研究者数据失败: {e}")
	# 在后台启动初始化
	if researcher_manager and twitter_api:
	    # 使用线程在后台初始化数据，不阻塞应用启动
	    init_thread = threading.Thread(target=initialize_researcher_data, daemon=True)
	    init_thread.start()
	    logger.info("🔄 研究者数据初始化已在后台启动...")
	# API路由
	@app.route('/')
	def index():
	    """主页路由 - 返回HTML模板"""
	...
	def fetch_historical_content(researcher_id):
	    ...
	    try:
	        ...
	        max_results = data.get('max_results', 50)  # 减少默认最大结果数
	        conn = sqlite3.connect('research_platform.db')
	        cursor = conn.cursor()
	        cursor.execute('SELECT name, x_account FROM researchers WHERE id = ?', (researcher_id,))
	        researcher = cursor.fetchone()
	        ...
	        if not twitter_api or not twitter_api.client:
	            logger.error(f"❌ Twitter API未初始化，无法抓取 {name} 的历史内容")
	            conn.close()
	            return jsonify({'error': 'Twitter API not initialized'}), 500
	        # 获取历史推文
	        logger.info(f"🔄 正在请求 {name} 的历史推文...")
	        ...
	        if not tweets:
	            logger.info(f"ℹ️ 未找到 {name} 在指定时间范围内的推文")
	            conn.close()
	            period = f"{start_date} 到 {end_date}" if start_date else f"所有时间到 {end_date}"
	            return jsonify({
	                'message': f'未找到 {name} 在 {period} 期间的历史内容',
	                'new_content_count': 0,
	                'total_fetched': 0,
	                'period': period
	            })
	        logger.info(f"✅ 成功获取 {name} 的 {len(tweets)} 条推文，开始保存到数据库...")
	        new_content_count = 0
	        for tweet in tweets:
	            # 存储媒体URL为JSON字符串
